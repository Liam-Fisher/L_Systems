...
class System {
    update(act) {
        if (this.sCtx.stage === 'idle')
            return false;
        this.sCtx.stage = act;
        this.sCtx.index = this.sCtx.indices.get(act) ?? -1;
        if (++this.sCtx.index === this.sCtx.amounts.get(act))
            return !this.sCtx.indices.delete(act); // fancy way of saying false
        else
            return this.sCtx.indices.set(act, this.sCtx.index).has(act);
    }
    match() {
    ...
    reset() {
        ...
        if (this.gCtx.letters.length >= this.gCtx.constants?.letterLimit)
            return true;
    }
}
