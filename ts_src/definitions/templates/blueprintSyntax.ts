...
import {systemOutTypes, networkOutTypes, maxClass, fileExtension} from '../static/fileio';
...
export type systemFiles<ctx extends _l.loaded> = {
    [Tf in (keyof systemOutTypes)]?: (maxformat<maxClass<Tf>>&ctxType<ctx, input, {args?: number[]}>&file&Partial<obj>)[]
};
export type networkFiles<Tx> = {
[T in keyof networkOutTypes]?: (Tx&maxformat<maxClass<T>>&fileext<fileExtension<T>>)[]
};
